...
(*                                                                            *)
(*     finsupp f == the finite set of inputs where f differs from def         *)
...
Proof. rewrite mem_finsupp negbK; exact/eqP. Qed.
...
Lemma finsupp_mkffun fb xs :
  finsupp (mkffun fb xs) = fset [seq x <- xs | fb x != def x].
Proof.
apply/eq_fset=> x; rewrite mem_finsupp in_fset mem_filter mkffunE.
by rewrite andbC; case: ifP=> //; rewrite eqxx.
Qed.

Lemma finsupp_mkffun_sub fb (X : {fset T}) : finsupp (mkffun fb X) `<=` X.
Proof.
by apply/fsubsetP => x; rewrite finsupp_mkffun in_fset mem_filter; case/andP.
Qed.
...
Arguments finsuppPn {T S def f x}.
...
Definition mapf (g : S -> R) (f : ffun def) : ffun (g \o def) :=
  mkffun (g \o f) (finsupp f).

Lemma mapfE (g : S -> R) (f : ffun def) x :
  mapf g f x = g (f x).
Proof.
rewrite /mapf mkffunE mem_finsupp /=.
by case: eqP=> //= ->.
Qed.
...
rewrite /mapf val_mkffun mkfmapfpE mapmE mem_finsupp /= /appf /=.
rewrite inj_eq //.
...
#[deprecated(since="extructures 0.6.0", note="use finsupp instead")]
Notation supp := finsupp (only parsing).
#[deprecated(since="extructures 0.6.0", note="use mem_finsupp instead")]
Notation mem_supp := mem_finsupp (only parsing).
#[deprecated(since="extructures 0.6.0", note="use mem_finsuppN instead")]
Notation mem_suppN := mem_finsuppN (only parsing).
#[deprecated(since="extructures 0.6.0", note="use finsuppPn instead")]
Notation suppPn := finsuppPn (only parsing).
#[deprecated(since="extructures 0.6.0", note="use finsupp0 instead")]
Notation supp0 := finsupp0 (only parsing).
#[deprecated(since="extructures 0.6.0", note="use finsupp_mkffun instead")]
Notation supp_mkffun := finsupp_mkffun (only parsing).
#[deprecated(since="extructures 0.6.0", note="use finsupp_mkffun_sub instead")]
